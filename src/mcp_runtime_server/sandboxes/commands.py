...
async def clone_repository(
    url: str, target_dir: Path, branch: Optional[str], env_vars: Dict[str, str]
) -> None:
    ...
    try:
        logger.debug(
            {"event": "preparing_clone", "url": url, "target_dir": str(target_dir)}
        )
        ...
        process = await run_command(cmd, str(Path(target_dir).parent), env_vars)
        stdout, stderr = await process.communicate()
        ...
        logger.error({"event": "clone_error", "error": str(e)})
