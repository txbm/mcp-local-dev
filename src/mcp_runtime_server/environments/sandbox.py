...
def make_sandbox_dirs(root: Path) -> Tuple[Dict[str, Path], Dict[str, str]]:
    ...
    """
    logger.debug({
        "event": "creating_sandbox_directories",
        "root": str(root)
    })
    ...
    try:
        ...
        for name, path in dirs.items():
            ...
            logger.debug({
                "event": "created_sandbox_directory",
                "name": name,
                "path": str(path)
            })
...
def apply_security(root: Path) -> None:
    ...
    try:
        if platform.system() == "Linux":
            logger.debug({
                "event": "applying_unix_permissions",
                "root": str(root)
            })
            _apply_unix_permissions(root)
            logger.info({
                "event": "unix_permissions_applied",
                "root": str(root)
            })
    except Exception as e:
        logger.error({
            "event": "security_application_failed",
            "error": str(e),
            "root": str(root)
        })
...
def create_sandbox(root: Path) -> Sandbox:
    ...
    """
    try:
        # Create directory structure and get environment
...
def cleanup_sandbox(sandbox: Sandbox) -> None:
    ...
    try:
        if sandbox.root.exists():
            logger.debug({
                "event": "cleaning_sandbox",
                "root": str(sandbox.root)
            })
            shutil.rmtree(sandbox.root, ignore_errors=True)
            logger.info({
                "event": "sandbox_cleaned",
                "root": str(sandbox.root)
            })
    except Exception as e:
        logger.error({
