...
async def run_vitest(env: Environment) -> Dict[str, Any]:
    ...
    logger.debug(
        {"event": "starting_vitest_run", "work_dir": str(env.sandbox.work_dir)}
    )

    cmd_prefix = (
        "bun"
        if env.runtime_config.name == Runtime.BUN
        else "node --experimental-vm-modules"
    )

    # Install coverage dependency if needed
    await run_sandboxed_command(
        env.sandbox, "npm install -D @vitest/coverage-v8 --legacy-peer-deps"
    )
    ...
    try:
        ...
        if coverage_part:
            try:
                ...
                logger.warning(
                    {"event": "vitest_coverage_parse_error", "error": str(e)}
                )
...
async def check_vitest(env: Environment) -> bool:
    ...
    config_exists = any(
        env.sandbox.work_dir.glob(p) for p in ["vitest.config.js", "vitest.config.ts"]
    )
